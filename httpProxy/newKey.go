...
// RandKey 生成 256个byte随机组合的 密码，最后会使用base64编码为字符串存储在配置文件中
// 不能出现任何一个重复的byte位，必须又 0-255 组成，并且都需要包含
func RandKey() (keyBase string) {
	...
	keyBase = base64.StdEncoding.EncodeToString(k[:])
	return
}
...
// ParseBaseKey 解析采用base64编码的字符串获取密码
func ParseBaseKey(baseKey string) (k *Key, err error) {
	bs, err := base64.StdEncoding.DecodeString(strings.TrimSpace(baseKey))
